...
- Keep the total answer length within 100 words.
...
async def search_ddg(query):
    ...
    try:
        search_results = await loop.run_in_executor(None, lambda: list(ddgs.text(query, max_results=3)))
...
def get_search_query(query, prev_messages, client):
    ...
    response = client.responses.create(
        ...
        input=[{"role": "system", "content": system_prompt_search}, *new_messages],
...
def get_cited_answer(query, context_block, prev_messages, client):
    ...
    response = client.responses.create(
        model=os.getenv("MODEL_NAME"),
        input=[{"role": "system", "content": system_prompt_cited_answer}, *new_messages],
    ...
    new_prev_messages = prev_messages + [{"role": "assistant", "content": answer}]
